...
Dung \cite{DBLP:journals/inffus/VuBA11} proposed a solution for solving the SLAM with DATMO in outdoor environment. This approach used an additional sensor to provide a more reliable and accurate model, in this case the short-range radar was chosen. ICP was used to reduce the odometry error, several alternatives of such method were tested. Including scan matching, that were not satisfactory. Thus the maximum likelihood estimation was adopted in his work. 
...
In our method, we use as input an occupancy grid, %in which the scan alignment is not needed, since the occupancy grid 
