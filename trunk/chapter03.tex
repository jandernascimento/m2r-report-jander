...
The main goal of this research is to distinguish dynamic parts of the scene from the static parts. The scene is observed from two laser scanners installed in the front bumper of a demonstrator vehicle, so called testbed. This distinction between static and dynamic parts is to be made both when vehicle is static and when it is moving.
...
Our testbed is a Toyota Lexus car equipped with two LIDAR lasers scanner (Check Section~\ref{sec:testbed} for images) installed on the front bumper (Figure~\ref{fig:demonstrator:birdeye}).
...
As we are about to see in the next Sections, the algorithm developed in this document uses data from two main sensorst: the LIDARs (explained previously) and MTi-G sensor (motion sensor).
...
The quaternions at two time steps give the direction of the vehicle in radians that we can use along with the time difference $dt$ to calcualte the angular velocity $\omega_t$.

The angular velocity and translational velocty (calculated from the two components) are used to find the displacement undrgone by the vehicle from time $t-1$ to $t$.
...
\section{summary}
In this chapter we have formaly detailed the position of our module in the overall system architecture and have detailed the technique that we have developed. We have explained how the inputs from sensors and fusion module are used to make inferences about the static and dynamic parts of the environment. In next chapter we will give implementation detials of the techniques developed in this chapter along with experimental results.
