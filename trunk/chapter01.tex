...
This will be done by providing a theorerical framework to classify the environment in a high level manner, separating static and dynamic objects with the information acquired from the sensors.
...
The result of mapping, as general knowledge, gives us information about a region. It represents the action of recording the disposition of the static objects of a region. This representation can be used later for exploration purpose. In a geological map the static objects are rivers, montains, canyons, etc. 

Localization and Mapping are really important tools for a robot, but they insuficient in case of existing moving objects sharing the same environment with the robot. This is due to the risk of collition between the moving objects and the robot, for this reason its necessary to detect (be aware of the existance of such object) and track (know its motion model) the object.
...
From the probabilistic point of view, this implies in estimate the joint probability distribution (Equation \ref{jpd:discrete}). Where the $u$ represents the observations and $z$ the measurements.
